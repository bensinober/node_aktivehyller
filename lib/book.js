...
Book.prototype.fromTnr = function(tnr, callback) {
  var self = this;
  self.tnr = parseInt(tnr);
  self.uri = String(self.base_uri) + self.tnr;
  self.bookId = self.uri;
  mySparqlService.getFormatAndTitle(self.uri, function(err, data) {
    if (err) { callback(err); }
    console.log(data);
    if (data){
...
Book.prototype.random = function(callback) {
  var self = this;
  mySparqlService.fetchRandomBook(function(err, data) {
    if (err) { callback(err); }
    self.bookId = data.tnr;
    mySparqlService.getFormatAndTitle(String(self.bookId), function(err, data) {
      if (err) { callback(err); }
      if (data){
        self.formats = data.formats;
        self.title = data.title;
...
Book.prototype.populate = function(callback) {
  ...
  mySparqlService.fetchBookInfo(this.uri, function(err, data) {
    ...
    self.coverUrl = data.bindings.coverUrl;
